...
func (s *Server) registerRoutes() *mux.Router {
	...
	r.Handle("/events", s.authMiddleware(s.createEventHandler(), false, true)).Methods("PUT")
	r.Handle("/events/{eventKey}/info", s.eventHandler()).Methods("GET")
	r.Handle("/events/{eventKey}/matches", s.matchesHandler()).Methods("GET")
	r.Handle("/events/{eventKey}/matches", s.authMiddleware(s.createMatchHandler(), false, true)).Methods("PUT")
